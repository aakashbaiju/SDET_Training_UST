...
test("TC03 - Validate error for invalid Contact Number", async ({ page }) => {
    ...
    const errorMsg = page.getByText("Invalid Phone Number");
    await expect(errorMsg).toBeVisible();
    await expect(errorMsg).toHaveText("Invalid Phone Number");
});
